...
TARGET_FILE=".jemaos_factory_install"
BOARD_FILE="jemaos_board_hooks.sh"
LOG_FILE="/var/log/jemaos_factory_install.log"
  ...
  if [ -n "${target_devs}" ]; then
    for target_disk in $target_devs ; do
      ...
      if [ $? -ne 0 ]; then
         ...
         display_boot_message jemaos_install_failure 'zh-CN en'
         wait_for_key_to_reboot
         exit 1
      fi    
      log_install "end install"
      jemaos_post_install ${target_disk} 
      display_boot_message jemaos_install_success 'zh-CN en'
      wait_for_key_to_reboot
      exit 0
    done
    log_install "can't find the target disk"
    display_boot_message jemaos_target_failure 'zh-CN en'
    wait_for_key_to_reboot
    exit 1
  fi
}
